    ...
    for column in TIMESTAMP_COLUMNS:
        if column in parsed.columns:
            parsed[column] = pd.to_datetime(
                parsed[column], errors="coerce", utc=True
            ).dt.tz_convert(None)
    return parsed
...
def clean_sessions(sessions: pd.DataFrame) -> pd.DataFrame:
    ...
    if {"login_time", "logout_time"}.issubset(cleaned.columns):
        cleaned = cleaned[
            cleaned["logout_time"].isna() | (cleaned["logout_time"] >= cleaned["login_time"])
        ]
...
def clean_subscriptions(subscriptions: pd.DataFrame) -> pd.DataFrame:
    ...
    cleaned = (
        cleaned.drop_duplicates(subset=["subscription_id"], keep="last")
        if "subscription_id" in cleaned.columns
        else cleaned.drop_duplicates()
    )
